# Hi, I'm Berkay

📍 **London ↔ Bristol** | 🛡️ **Systems Architect & Polymath** | 🚀 **Founder @ TensorSphere**

![Go](https://img.shields.io/badge/-Go-00ADD8?style=flat&logo=go&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white)
![AWS](https://img.shields.io/badge/-AWS-232F3E?style=flat&logo=amazon-aws&logoColor=white)
![Terraform](https://img.shields.io/badge/-Terraform-7B42BC?style=flat&logo=terraform&logoColor=white)
![Cloudflare](https://img.shields.io/badge/-Cloudflare-F38020?style=flat&logo=cloudflare&logoColor=white)

> Architecting the trust layer for the Agentic Era. I synthesize distributed systems, applied cryptography, and economic strategy to build sovereign, secure infrastructure.

## Current Projects

- 🔐 **[Agent Auth Protocol](https://github.com/agent-auth-protocol)** - Enterprise-grade identity and access management (IAM) for the autonomous economy.
- 💸 **[MCP AWS Cost Explorer](https://github.com/berkayyildirim/mcp-aws-cost-explorer)** - Secure, read-only AWS Cost Explorer access for AI orchestration layers (Claude Desktop/LangChain).
- 🏛️ **[SGX Attestation Stub](https://github.com/berkayyildirim/sgx-attestation-stub)** - Python simulator for SGX Hardware Attestation for Sovereign AI.

## Experience

- ☁️ **MindWalk** - Software Engineer. Building zero-trust cloud infrastructure and passwordless federated identity systems.
- 🌟 **Huawei (Ex)** - Software Engineer. Rescued critical international AI platforms; awarded the *Sparkling Star Award* for dynamic problem-solving and resilience.
- 🚀 **TensorSphere** - Founder. Consulting on scalable cloud architecture and data sovereignty for AI integrations.

## Recognition

- 🎙️ **[The Alan Turing Institute](https://youtu.be/H0jlpiNTdo8?si=tZRLqk5IW5g-bZos)** - Invited speaker. Presented *"Architecting for Autonomy: A Federated and Explainable AI Blueprint for UK Public Services"* at the International Conference on Trustworthy Digital Infrastructure. *(Paper in publishing process by the IET).*
- 📜 **UK Government (DSIT)** - Submitted expert policy feedback advising on the Code of Practice for Enterprise Connected Device Security.
- 🏅 **MIET** - Member of the Institution of Engineering and Technology & selected foundational Career Mentor.

## Connect

[![X](https://img.shields.io/badge/@berkayildi-000000?style=flat&logo=x&logoColor=white)](https://twitter.com/berkayildi)
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/by-berkayyildirim)

## Philosophy

> "I don’t just write code; I anticipate structural vulnerabilities—whether in software architecture, market economics, or national data sovereignty—and engineer resilient solutions."

<details>
<summary>Random Facts</summary>

- **Explainable AI (XAI):** Holds an MSc in AI, specializing in using LLMs to translate "Black Box" medical diagnostics into human-readable insights.
- **Markets & Capital:** Deeply focused on macroeconomics, algorithmic trading, ETFs, and how global capital flows dictate technological paradigm shifts.
- **Systems Thinking:** Treating software engineering not as an isolated discipline, but as a critical intersection of ethics, security, and global policy.
- **Mentorship:** Active Computer Science mentor, bridging the gap between rigorous academic theory and high-stakes production environments.

</details>
